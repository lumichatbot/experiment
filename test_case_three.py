...
TEST_CASE = {
    "intent_text": "Please add a firewall and an IDS from Iperf client to server",
    "origin": "iperf client",
    "destination": "iperf server",
    "middleboxes": ["\"firewall\"", "\"ids\""]
}
...
if __name__ == "__main__":
    ...
    nile_filename = 'res/nile/{}_nile.nil'.format(datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d_%H%M%S'))
    ...
    script_filename = 'res/sonata/{}_sonata.sh'.format(datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d_%H%M%S'))
